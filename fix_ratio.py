...
class _MultiLiveViewScreenState extends State<MultiLiveViewScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // Use LayoutBuilder to get exact constraints
      body: LayoutBuilder(
        builder: (context, constraints) {
          ...
          // Fixed 5 rows approach
          final double cellHeight = totalHeight / fixedRows;
          final double cellWidth = totalWidth / _gridColumns;
          
          // Build a custom grid layout
          return Column(
            ...
            children: [
              // Main grid container that will use 100% of available height
              Container(
                width: totalWidth,
                height: totalHeight,
                // Fixed grid layout with rows and columns
                child: Column(
                  mainAxisSize: MainAxisSize.max,
                  children: List.generate(fixedRows, (rowIndex) {
                    return Row(
                      mainAxisSize: MainAxisSize.max,
                      children: List.generate(_gridColumns, (colIndex) {
                        ...
                        // Create a fixed-size container for each cell
                        return Container(
                          width: cellWidth,
                          height: cellHeight,
                          child: _buildCameraCell(index),
                        );
                      }),
                    );
                  }),
