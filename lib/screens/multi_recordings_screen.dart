...
class _MultiRecordingsScreenState extends State<MultiRecordingsScreen>
  ...
  String _cameraSearchQuery = '';
  final Map<Camera, List<String>> _cameraRecordings = {};
  final Map<Camera, String> _cameraErrors =
      {}; // Her kamera için ayrı hata mesajları
  final Map<Camera, String> _cameraDateFormats =
      {}; // Her kamera için hangi tarih formatının çalıştığını sakla
  final Map<Camera, String> _cameraSelectedDevice =
      {}; // Her kamera için seçilen cihaz MAC adresi
  ...
  final Map<Camera, Map<String, String>> _cameraDeviceErrors =
      {}; // Camera -> DeviceMAC -> error message
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      print(
          '[MultiRecordings] PostFrameCallback - Loading cameras and handling arguments');
  ...
  DateTime? _parseTimestampFromFilename(String filename) {
    ...
    try {
      ...
      if (nameWithoutExt.length >= 14 &&
          RegExp(r'^\d+$').hasMatch(nameWithoutExt)) {
      ...
      if (nameWithoutExt.contains('-') &&
        ...
        try {
          ...
          if (hour >= 0 &&
            ...
            final timestamp = DateTime(selectedDate.year, selectedDate.month,
                selectedDate.day, hour, minute, second);
            return timestamp;
          }
        } catch (e) {
          print(
              '[MultiRecordings] Error parsing time-only format for $nameWithoutExt: $e');
        }
      }
      ...
      if (nameWithoutExt.contains('__')) {
        ...
        if (parts.length == 2 && parts[0].length == 6 && parts[1].length == 6) {
          try {
            ...
            final selectedDate = _selectedDay ?? DateTime.now();
            final timestamp = DateTime(selectedDate.year, selectedDate.month,
                selectedDate.day, startHour, startMinute, startSecond);

            print(
                '[MultiRecordings] Parsed MP4 time range format: $nameWithoutExt -> $timestamp');
            return timestamp;
          } catch (e) {
            print(
                '[MultiRecordings] Error parsing MP4 time range format for $nameWithoutExt: $e');
          }
        }
      }
  ...
  void _loadAvailableCameras() {
    ...
    setState(() {
      _availableCameras = cameras
          .where(
              (camera) => camera.ip.isNotEmpty && camera.recordUri.isNotEmpty)
          .toList();

      print(
          '[MultiRecordings] Available cameras after filtering: ${_availableCameras.length}');
      for (final camera in _availableCameras) {
        print(
            '[MultiRecordings] Camera: ${camera.name}, IP: ${camera.ip}, RecordUri: ${camera.recordUri}');
      }

      // Handle pending camera selection from route arguments
      if (_pendingCameraSelection != null) {
        print(
            '[MultiRecordings] Looking for camera: "$_pendingCameraSelection"');
        ...
        final targetCamera = _availableCameras.firstWhere(
          ...
          orElse: () {
            print(
                '[MultiRecordings] Exact match not found, trying partial matches...');
            ...
            partialMatch = _availableCameras.cast<Camera?>().firstWhere(
                  (camera) =>
                      ...
                      _pendingCameraSelection!.toLowerCase(),
                  orElse: () => null,
                );
            if (partialMatch != null) {
              print(
                  '[MultiRecordings] Found case-insensitive match: "${partialMatch.name}"');
              return partialMatch;
            }
            ...
            partialMatch = _availableCameras.cast<Camera?>().firstWhere(
                  (camera) => camera!.name
                      .toLowerCase()
                      .contains(_pendingCameraSelection!.toLowerCase()),
                  orElse: () => null,
                );
            if (partialMatch != null) {
              print(
                  '[MultiRecordings] Found partial match (contains): "${partialMatch.name}"');
              return partialMatch;
            }

            // Strategy 3: Check if selection contains the camera name
            partialMatch = _availableCameras.cast<Camera?>().firstWhere(
                  (camera) => _pendingCameraSelection!
                      .toLowerCase()
                      .contains(camera!.name.toLowerCase()),
                  orElse: () => null,
                );
            if (partialMatch != null) {
              print(
                  '[MultiRecordings] Found reverse partial match: "${partialMatch.name}"');
              return partialMatch;
            }

            print(
                '[MultiRecordings] No match found, using first camera or dummy');
            return _availableCameras.isNotEmpty
        ...
        if (targetCamera.index != -1) {
          ...
          print(
              '[MultiRecordings] Pre-selected camera from route arguments: ${targetCamera.name}');
        }
          ...
          content: Column(
            ...
            children: [
              ...
              ...camera.currentDevices.entries.map((entry) {
                ...
                final deviceName =
                    device?.deviceName ?? deviceMac.toUpperCase();
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        Text(
                            'IP: ${deviceIp.isNotEmpty ? deviceIp : deviceInfo.deviceIp}'),
                        Text(
    ...
    for (var camera in _selectedCameras) {
      ...
      if (camera.currentDevices.length > 1) {
        for (var deviceMac in camera.currentDevices.keys) {
          ...
          try {
            ...
            if (deviceInfo != null && deviceInfo.deviceIp.isNotEmpty) {
              recordingsUrl =
                  'http://${deviceInfo.deviceIp}:8080/Rec/${camera.name}/';
            }
          }

          if (recordingsUrl != null) {
            await _refreshRecordingsForDevice(camera, deviceMac, recordingsUrl,
                selectedDayFormatted, selectedDayFormattedAlt);
          }
        }
      } else {
        ...
        if (selectedDeviceMac != null &&
            camera.currentDevices.containsKey(selectedDeviceMac)) {
          try {
            device = cameraDevicesProvider.devices.values.firstWhere(
              (d) =>
                  d.macAddress == selectedDeviceMac ||
                  d.macKey == selectedDeviceMac,
            );
          } catch (e) {
        ...
        if (device != null) {
          final recordingsUrl =
              'http://${device.ipv4}:8080/Rec/${camera.name}/';
          await _refreshSingleCameraRecordings(camera, recordingsUrl,
              selectedDayFormatted, selectedDayFormattedAlt);
        }
      }
    }
  ...
  Future<void> _refreshRecordingsForDevice(
      Camera camera,
      String deviceMac,
      String recordingsUrl,
      String formattedDate,
      String formattedDateAlt) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (foundDate != null) {
          ...
          if (dateResponse.statusCode == 200) {
            ...
            final m3u8Recordings = m3u8RegExp
                .allMatches(dateHtml)
                .map((m) => m.group(1)!)
                .toList();
            final mp4Recordings =
            ...
            if (newRecordings.length > existingRecordings.length) {
              print(
                  '[MultiRecordings] New recordings found for ${camera.name}/$deviceMac: ${newRecordings.length - existingRecordings.length} new');
              if (mounted) {
                setState(() {
      ...
      print(
          '[MultiRecordings] Error refreshing recordings for ${camera.name}/$deviceMac: $e');
    }
  }

  // Refresh recordings for a single-device camera
  Future<void> _refreshSingleCameraRecordings(
      Camera camera,
      String recordingsUrl,
      String formattedDate,
      String formattedDateAlt) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (foundDate != null) {
          ...
          if (dateResponse.statusCode == 200) {
            ...
            if (newRecordings.length > existingRecordings.length) {
              print(
                  '[MultiRecordings] New recordings found for ${camera.name}: ${newRecordings.length - existingRecordings.length} new');
              if (mounted) {
                setState(() {
      ...
      print(
          '[MultiRecordings] Error refreshing recordings for ${camera.name}: $e');
    }
  }
  ...
  void _updateRecordingsForSelectedDay() {
    ...
    for (var camera in _cameraDeviceRecordings.keys) {
      if (!_selectedCameras.contains(camera) &&
          !camerasToRemove.contains(camera)) {
        camerasToRemove.add(camera);
      }
    }
    ...
    print(
        '[MultiRecordings] Loading recordings for ${camerasToLoad.length} cameras (${_selectedCameras.length} total selected)');
    ...
    for (var camera in camerasToLoad) {
      ...
      print(
          '[MultiRecordings] Camera currentDevices: ${camera.currentDevices.keys.join(", ")}');

      // If camera is on multiple devices, load recordings from ALL devices
      if (camera.currentDevices.length > 1) {
        print(
            '[MultiRecordings] Camera ${camera.name} is on ${camera.currentDevices.length} devices, loading from all');
        ...
        for (var deviceMac in camera.currentDevices.keys) {
          ...
          try {
            ...
            final recordingsUrl =
                'http://${device.ipv4}:8080/Rec/${camera.name}/';
            print(
                '[MultiRecordings] Loading from device ${device.deviceName ?? deviceMac} (IP: ${device.ipv4})');
            final future = _loadRecordingsForCameraDevice(camera, deviceMac,
                recordingsUrl, selectedDayFormatted, selectedDayFormattedAlt);
            futures.add(future);
          } catch (e) {
            ...
            if (deviceInfo != null && deviceInfo.deviceIp.isNotEmpty) {
              ...
              final recordingsUrl =
                  'http://${deviceInfo.deviceIp}:8080/Rec/${camera.name}/';
              print(
                  '[MultiRecordings] Loading from device $deviceMac (IP: ${deviceInfo.deviceIp})');
              final future = _loadRecordingsForCameraDevice(camera, deviceMac,
                  recordingsUrl, selectedDayFormatted, selectedDayFormattedAlt);
              futures.add(future);
            }
        ...
        if (device != null) {
          final recordingsUrl =
              'http://${device.ipv4}:8080/Rec/${camera.name}/';
          print(
              '[MultiRecordings] Single device - Loading from ${device.ipv4}');
          final future = _loadRecordingsForCamera(camera, recordingsUrl,
              selectedDayFormatted, selectedDayFormattedAlt);
          futures.add(future);
        } else {
          print(
              '[MultiRecordings] ERROR: No device found for camera ${camera.name}');
        }
      }
    }
    ...
    Future.wait(futures).then((_) {
      ...
      print(
          '[MultiRecordings] All recordings loaded. Checking for target time: $_pendingTargetTime');

      // If we have a target time, find and play the recording before that time
      if (_pendingTargetTime != null && _activeCamera != null) {
        print(
            '[MultiRecordings] Calling _findAndPlayRecordingBeforeTime with camera: ${_activeCamera!.name}, targetTime: $_pendingTargetTime');
        _findAndPlayRecordingBeforeTime(_activeCamera!, _pendingTargetTime!);
        _pendingTargetTime = null; // Clear after use
      } else {
        print(
            '[MultiRecordings] No target time or active camera. TargetTime: $_pendingTargetTime, ActiveCamera: ${_activeCamera?.name}');
      }
    }).catchError((error) {
      setState(() {
        _isLoadingRecordings = false;
  ...
  Future<void> _loadRecordingsForCamera(Camera camera, String recordingsUrl,
      String formattedDate, String formattedDateAlt) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print(
            '[MultiRecordings] Looking for date: $formattedDate or $formattedDateAlt');
        ...
        if (dates.contains(formattedDate)) {
          ...
          foundDate = formattedDateAlt;
          _cameraDateFormats[camera] =
              formattedDateAlt; // Çalışan formatı kaydet
        }
        ...
        if (foundDate != null) {
          ...
          if (dateResponse.statusCode == 200) {
            ...
            print(
                '[MultiRecordings] Failed to load date folder: ${dateResponse.statusCode}');
          }
        } else {
          // Bu tarih için kayıt yok
          print(
              '[MultiRecordings] No recordings found for date $formattedDate or $formattedDateAlt for camera ${camera.name}');
          if (mounted) {
            setState(() {
        ...
        print(
            '[MultiRecordings] Failed to load recordings URL: ${response.statusCode}');
        throw Exception('Failed to load recordings: ${response.statusCode}');
      }
    } catch (e) {
      print(
          '[MultiRecordings] Error loading recordings for ${camera.name}: $e');
      if (mounted) {
        setState(() {
  ...
  Future<void> _loadRecordingsForCameraDevice(
      Camera camera,
      String deviceMac,
      String recordingsUrl,
      String formattedDate,
    ...
    try {
      print(
          '[MultiRecordings] Loading recordings for camera: ${camera.name} from device: $deviceMac');
      print('[MultiRecordings] Recordings URL: $recordingsUrl');

      final response = await http.get(Uri.parse(recordingsUrl));
      print(
          '[MultiRecordings] Response status for $deviceMac: ${response.statusCode}');
      ...
      if (response.statusCode == 200) {
        ...
        if (foundDate != null) {
          ...
          if (dateResponse.statusCode == 200) {
            ...
            print(
                '[MultiRecordings] Found ${allRecordings.length} recordings for ${camera.name} from device $deviceMac');
          ...
          print(
              '[MultiRecordings] No recordings for $formattedDate from device $deviceMac');
          if (mounted) {
            setState(() {
              _cameraDeviceRecordings[camera] ??= {};
              _cameraDeviceRecordings[camera]![deviceMac] = [];
  ...
  void _findAndPlayRecordingBeforeTime(Camera camera, String targetTime) {
    print(
        '[MultiRecordings] Finding recording before time: $targetTime for camera: ${camera.name}');
    ...
    int seekSeconds =
        0; // How many seconds to seek forward in the found recording
    ...
    for (final recording in recordings) {
      ...
      if (timestamp != null) {
        ...
        print(
            '[MultiRecordings] Recording: $recording, time: $hour:$minute:$second (${recordingTimeInSeconds}s)');
        ...
        if (recordingTimeInSeconds < targetTimeInSeconds) {
          ...
          if (recordingTimeInSeconds > bestRecordingTime) {
            ...
            seekSeconds =
                (targetTimeInSeconds - recordingTimeInSeconds).round();
          }
        }
      } else {
        print(
            '[MultiRecordings] Could not parse time from recording: $recording');
      }
    }
    ...
    if (bestRecording != null) {
      ...
      print(
          '[MultiRecordings] Will seek $seekSeconds seconds into the recording');
      ...
      if (recordings.isNotEmpty) {
        print(
            '[MultiRecordings] Playing first available recording: ${recordings.first}');
  ...
  void _openVideoPlayerPopup(Camera camera, String recording) {
    print(
        '[MultiRecordings] _openVideoPlayerPopup called with seekTime: $_pendingSeekTime');
  ...
  void _toggleMultiSelectionMode() {
    print(
        '[Toggle] Toggling multi-selection mode. Before: $_isMultiSelectionMode');
    setState(() {
      _isMultiSelectionMode = !_isMultiSelectionMode;
      if (!_isMultiSelectionMode) {
        print(
            '[Toggle] Clearing selected downloads. Count was: ${_selectedForDownload.length}');
  ...
  void _downloadSelectedRecordings() async {
    ...
    print(
        '[Download] Selected recordings count: ${_selectedForDownload.length}');
    ...
    if (!permissionStatus) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Storage permission required to download recordings')));
      return;
    }
    ...
    for (final recordingUrl in _selectedForDownload) {
      ...
      try {
        ...
        print(
            '[Download] HTTP Response body length: ${response.bodyBytes.length}');
      ...
      if (Platform.isAndroid) {
        ...
        if (Platform.isMacOS || Platform.isWindows || Platform.isLinux) {
          ...
          if (!shouldShowPicker) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                content: Text(
                    'File picker will be implemented soon. Using Downloads folder.')));
            final dir = await getDownloadsDirectory();
      ...
      for (final rec in group) {
        final fileType = rec.recording.toLowerCase().contains('.mkv')
            ...
            : rec.recording.toLowerCase().contains('.m3u8')
                ...
                : 'UNKNOWN';
        print(
            '[MultiRecordings]   ${rec.camera.name}: ${rec.recording} ($fileType)');
      }
    }
  ...
  void _openMultiWatchScreen(List<RecordingTime> recordingGroup) {
      ...
      if (recording.deviceMac != null) {
        ...
        try {
          final device = cameraDevicesProvider.devices.values.firstWhere(
            (d) =>
                d.macAddress == recording.deviceMac ||
                d.macKey == recording.deviceMac,
          );
          cameraDeviceIps[recording.camera] = device.ipv4;
        } catch (e) {
          // Try from currentDevices
          final deviceInfo =
              recording.camera.currentDevices[recording.deviceMac];
          if (deviceInfo != null && deviceInfo.deviceIp.isNotEmpty) {
        ...
        final device =
            cameraDevicesProvider.getDeviceForCamera(recording.camera);
        if (device != null) {
          cameraDeviceIps[recording.camera] = device.ipv4;
        }
    ...
    print(
        '[MultiRecordings] Opening MultiWatch with ${cameraRecordings.length} cameras');
    print(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            tooltip:
                _isMultiSelectionMode ? 'Cancel Selection' : 'Multi Select',
            onPressed: () {
              print(
                  '[Button] Multi-select toggle pressed! Current mode: $_isMultiSelectionMode');
          ...
          if (_isMultiSelectionMode)
            IconButton(
              ...
              onPressed: _selectedForDownload.isNotEmpty
                  ? () {
                      print(
                          '[Button] Download button pressed! Selected count: ${_selectedForDownload.length}');
                    ...
                    children: [
                      ...
                      Container(
                        constraints: BoxConstraints(
                          maxHeight:
                              sidebarHeight * 0.45, // 45% of sidebar height
                          minHeight:
                              250, // Minimum height to keep calendar readable
                        ),
  ...
  Widget _buildRecordingsList() {
    ...
    print(
        '[MultiRecordings] Total tabs calculated: $totalTabs (groups: ${groups.isNotEmpty ? 1 : 0}, cameras: ${_selectedCameras.length})');
    print(
        '[MultiRecordings] _cameraRecordings keys: ${_cameraRecordings.keys.map((c) => c.name).join(", ")}');
    print(
        '[MultiRecordings] _cameraDeviceRecordings keys: ${_cameraDeviceRecordings.keys.map((c) => "${c.name}(${_cameraDeviceRecordings[c]?.length ?? 0} devices)").join(", ")}');
          ...
          Expanded(
            child: TabBarView(
              children: [
                ...
                if (groups.isNotEmpty) _buildGroupRecordingsList(groups),
      ...
      if (camera.currentDevices.length > 1) {
        ...
        for (var deviceMac in camera.currentDevices.keys) {
          ...
          final deviceName = device?.deviceName ??
              deviceMac
                  .substring(deviceMac.length > 8 ? deviceMac.length - 8 : 0)
                  .toUpperCase();
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Text(
                            deviceName,
                            style: TextStyle(
                                fontSize: 9, color: Colors.orange.shade300),
                          ),
                        ],
                      ),
                    ],
    ...
    for (var camera in _selectedCameras) {
      ...
      if (camera.currentDevices.length > 1) {
        ...
        for (var deviceMac in camera.currentDevices.keys) {
          final recordings = deviceRecordings[deviceMac] ?? [];
          contents.add(
              _buildCameraDeviceRecordingsList(camera, deviceMac, recordings));
        }
      } else {
        // Single-device camera
        final recordings = _cameraRecordings[camera] ?? [];
  ...
  Widget _buildRecordingTile(Camera camera, String recording,
    ...
    return Card(
      ...
      child: ListTile(
        leading: Icon(
          isSelected && _isMultiSelectionMode
              ? Icons.check_circle
        ...
        trailing: Row(
          ...
          children: [
            IconButton(
              ...
              onPressed: _isMultiSelectionMode
                  ? null
                  : () => _selectRecordingWithDevice(
                      camera, recording, deviceMac, resolvedDeviceIp!),
            ),
            IconButton(
              icon: const Icon(Icons.download),
              onPressed: () =>
                  _downloadSingleRecording(camera, recording, recordingUrl),
            ),
          ],
        ),
        onTap: () => _selectRecordingWithDevice(
            camera, recording, deviceMac, resolvedDeviceIp!),
      ),
    );
  }
  ...
  void _selectRecordingWithDevice(
      Camera camera, String recording, String? deviceMac, String deviceIp) {
    if (_isMultiSelectionMode) {
  ...
  void _openVideoPlayerPopupWithDevice(
      Camera camera, String recording, String? deviceMac, String deviceIp) {
    print(
        '[MultiRecordings] Opening video player for ${camera.name} from device $deviceMac ($deviceIp)');
    print(
        '[MultiRecordings] _cameraDeviceDateFormats keys: ${_cameraDeviceDateFormats.keys.map((c) => c.mac).toList()}');
      ...
      if (deviceMac != null) {
        ...
        for (var entry in _cameraDeviceDateFormats.entries) {
          if (entry.key.mac == camera.mac) {
            dateFormat = entry.value[deviceMac];
            print(
                '[MultiRecordings] Found date format via MAC comparison: $dateFormat');
            break;
          }
        }
        if (dateFormat == null) {
    ...
    if (recordingUrl.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Could not determine device IP'),
  ...
  Widget _buildCameraDeviceRecordingsList(
      Camera camera, String deviceMac, List<String> recordings) {
    final cameraDevicesProvider =
        Provider.of<CameraDevicesProviderOptimized>(context, listen: false);
    ...
    final deviceIp =
        device?.ipv4 ?? camera.currentDevices[deviceMac]?.deviceIp ?? '';
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text('Error loading from $deviceName',
                style:
                    const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 32),
              child: Text(deviceError,
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text('No recordings on $deviceName',
                style: const TextStyle(fontSize: 16)),
            Text('IP: $deviceIp',
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return _buildRecordingTile(camera, recording,
                  deviceMac: deviceMac, deviceIp: deviceIp);
            },
          ),
        ),
      ],
  ...
  void _downloadSingleRecording(
      Camera camera, String recording, String recordingUrl) async {
    print(
        '[SingleDownload] Starting single download for: ${camera.name} - $recording');
    print('[SingleDownload] URL: $recordingUrl');
    print('[SingleDownload] Multi-selection mode: $_isMultiSelectionMode');

    if (_isMultiSelectionMode) {
      print(
          '[SingleDownload] In multi-selection mode, calling _selectRecording');
      _selectRecording(camera, recording);
      return;
    }
    ...
    try {
      ...
      print(
          '[SingleDownload] HTTP Response body length: ${response.bodyBytes.length}');
            ...
            LayoutBuilder(
              builder: (context, constraints) {
                ...
                final availableHeight =
                    constraints.maxHeight - 30; // Reserve space for title
                final cellSize = (constraints.maxWidth - 32) /
                    7; // 7 days per week, account for padding
                ...
                final totalNeededHeight =
                    headerHeight + daysOfWeekHeight + calendarBodyHeight;
                  ...
                  child: SingleChildScrollView(
                    child: TableCalendar(
                      ...
                      headerStyle: HeaderStyle(
                        ...
                        headerPadding:
                            EdgeInsets.symmetric(vertical: 4 * scaleFactor),
  ...
  Widget _buildGroupedCameraSelection() {
    ...
    print(
        '[MultiRecordings] Available cameras count: ${_availableCameras.length}');
    print(
